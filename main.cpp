...
struct Matrix
{
    ...
    static float Trace(Matrix m) //скалярное произведение
    {
        if(m.height != m.length)
        {
            cout<<"ERROR (Trace)";
    ...
    static Matrix Ort(Matrix m) //ортогонализация
    {
        if(m.height != m.length)
        {
            cout<<"ERROR (Ort)";
        ...
        Matrix ans = Matrix();
        ans.CreateBody(m.height, m.length);
        for(int i=0; i<ans.height; i++)
        {
            for(int j=0; j<ans.length; j++)
            {
                ans.body[i][j] = m.body[i][j];
            }
        }

        for(int k=0; k<ans.height; k++)
        {
            for (int i=0; i<k; i++)
            {
                Matrix a = Matrix();
                a.CreateBody(m.length, 1);
                for(int q=0; q<a.height; q++)
                {
                    for(int w=0; w<a.length; w++)
                    {
                        a.body[q][w] = m.body[k][q];
                    }
                }

                Matrix b = Matrix();
                b.CreateBody(1, ans.length);
                for(int q=0; q<b.height; q++)
                {
                    for(int w=0; w<b.length; w++)
                    {
                        b.body[q][w] = ans.body[i][w];
                    }
                }

                Matrix c = Matrix();
                c.CreateBody(ans.height, 1);
                for(int q=0; q<c.height; q++)
                {
                    for(int w=0; w<c.length; w++)
                    {
                        c.body[q][w] = ans.body[i][q];
                    }
                }

                for(int j=0; j<ans.length; j++)
                {
                    ans.body[k][j] -= Trace(a*b)/Trace(c*b)*ans.body[i][j];
                }
            }
        }
        return ans;
...
int main()
{
    ...
    {
        if(x == -1000) cout<<"Choose option: 1-sum 2-mult (multiki pokazhet (net)) 3-reverse 4-polynyam 5-Victori 6-spector-prorector 7-unitarnost' 8-ortogon:";
