...
def characteristic_polinom(m):
    if len(m) != len(m[0]):
        ...
        c = '+' + str(-m[0][0] * m[2][2] - m[1][1] * m[2][2] - m[1][1] * m[0][0] + m[0][2] * m[2][0] + m[1][2] * m[2][1] + m[1][
            0] * m[0][1]) + 'x'
        d = '+'+ str(m[0][0] * m[1][1] * m[2][2] + m[0][1] * m[1][2] * m[2][0] + m[0][2] * m[1][0] * m[2][1] - m[1][1] * m[0][
...
def what():  # функция, спрашивающая что делать с этими вашими матрицами
    v = int(input(
        'Введите "1" если хотите сложить матрицы, введите "2" если хотите умножить их, "3" если хотите найти обратную матрицу и "4" если хотите найти характеристический полином(блять))))0))000 '))
